...
import androidctl
from androidctl._version import __version__ as runtime_version

REPO_ROOT = Path(__file__).resolve().parents[3]
PACKAGE_ROOT = REPO_ROOT / "androidctl"
...
def test_package_version_matches_canonical_version() -> None:
    ...
    assert androidctl.__version__ == version
...
def test_pyproject_version_and_contract_pin_match_canonical_version() -> None:
    ...
    assert f"androidctld=={version}" in project["dependencies"]
